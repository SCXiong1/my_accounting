...
- 使用 Playwright 的 `webServer` 配置同时管理两个进程：后端 uvicorn (8080) 和前端 Vite dev server (5713)
...
- 本地已有 dev server 占用 5713/8080 端口时，Playwright 的 webServer 会尝试重用，可能导致测试使用错误的数据库。跑 E2E 前需停掉本地 dev server
