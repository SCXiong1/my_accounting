...
testAuth.describe('S11: 支出记录页筛选增强', () => {
  ...
  testAuth('选择时间范围后列表自动刷新', async ({ page }) => {
    ...
    // 记录当前列表数量
    const initialCount = await page.getByTestId('transaction-list-swipe-cell').count()
    ...
    // 列表应有数据
    const count = await page.getByTestId('transaction-list-swipe-cell').count()
  ...
  testAuth('时间筛选与其他筛选条件互不影响', async ({ page }) => {
    ...
    // 列表应有数据
    const countAfterYear = await page.getByTestId('transaction-list-swipe-cell').count()
    ...
    // 列表应有数据（种子数据在本月）
    const countAfterMonth = await page.getByTestId('transaction-list-swipe-cell').count()
