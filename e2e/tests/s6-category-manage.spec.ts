...
testAuth.describe('S6: 分类管理', () => {
  ...
  testAuth('编辑分类：点宠物 → 改名宠物猫 → 保存', async ({ page }) => {
    ...
    // 点击进入编辑
    await page
      .getByTestId('category-manage-swipe-cell')
      .filter({ hasText: `宠物A_${uid}` })
      .click()
  ...
  testAuth('删除分类：左滑 → 删除 → 确认 → 消失', async ({ page }) => {
    ...
    const swipeCell = page
      .getByTestId('category-manage-swipe-cell')
      .filter({ hasText: `待删_${uid}` })
    ...
    // 验证消失
    await expect(
      page.getByTestId('category-manage-swipe-cell').filter({ hasText: `待删_${uid}` }),
    ).not.toBeVisible()
  })
