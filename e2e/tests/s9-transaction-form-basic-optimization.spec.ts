...
testAuth.describe('S9: 记账表单基础优化', () => {
  ...
  testAuth('记账表单字段顺序：分类→标签→日期→备注→金额→提交', async ({ page }) => {
    ...
    const dateField = page.getByTestId('transaction-form-date')
    const noteField = page.getByTestId('transaction-form-note')
    const amountField = page.getByTestId('amount-field')
    const submitBtn = page.getByTestId('transaction-form-submit')
