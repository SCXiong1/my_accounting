...
testAuth.describe('S8: 页面导航稳定性', () => {
  ...
  testAuth('统计页面图表二次进入正常加载', async ({ page }) => {
    ...
    // 等待图表加载
    await page.waitForSelector('[data-testid="chart-bar"], [data-testid="chart-pie"]', { timeout: 10000 });
  ...
  testAuth('从我的页面返回支出记录，数据正常显示', async ({ page }) => {
    ...
    const cardCount = await page.locator('[data-testid="expense-card"]').count();
    ...
    const newCardCount = await page.locator('[data-testid="expense-card"]').count();
