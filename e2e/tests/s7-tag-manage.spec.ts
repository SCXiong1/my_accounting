...
testAuth.describe('S7: 标签管理', () => {
  ...
  testAuth('编辑标签：创建 → 点击 → 改名 → 确定', async ({ page }) => {
    ...
    // 点击 tag 进入编辑
    await page.getByTestId('tag-manage-tag').filter({ hasText: `编辑A_${uid}` }).click();
    await expect(dialog).toBeVisible();
  ...
  testAuth('删除标签：点 close → 确认 → 消失', async ({ page }) => {
    ...
    // 验证消失
    await expect(page.getByTestId('tag-manage-tag').filter({ hasText: `待删_${uid}` })).not.toBeVisible();
  });
});
