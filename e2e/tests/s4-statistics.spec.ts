...
testAuth.describe('S4: 统计页', () => {
  ...
  testAuth('分类点击跳转：点餐饮分类行 → /transactions?category_id=\\d+', async ({ page }) => {
    // "餐饮"在分类和标签区域都出现，用 testid 消歧
    await page.getByTestId('stats-category-row').filter({ hasText: '餐饮' }).click()

    // 验证 URL 跳转
    await page.waitForURL(/\/transactions\?category_id=\d+/)
    expect(page.url()).toMatch(/\/transactions\?category_id=\d+/)
  })

  testAuth('标签点击跳转：点标签行 → /transactions?tag_id=\\d+', async ({ page }) => {
    // 标签名在分类和标签区域都出现，用 testid 消歧
    await page.getByTestId('stats-tag-row').filter({ hasText: '餐饮' }).click()

    // 验证 URL 跳转
    await page.waitForURL(/\/transactions\?tag_id=\d+/)
    expect(page.url()).toMatch(/\/transactions\?tag_id=\d+/)
  })
})
